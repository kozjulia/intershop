package ru.yandex.practicum.store.showcase.configuration.constants;

import lombok.experimental.UtilityClass;

@UtilityClass
public class TemplateConstants {

    public static final String TEMPLATE_CART = "cart";
    public static final String TEMPLATE_MAIN = "main";
    public static final String TEMPLATE_ITEM = "item";
    public static final String TEMPLATE_ORDER = "order";
    public static final String TEMPLATE_ORDERS = "orders";
    public static final String TEMPLATE_ADD_ITEM = "add-item";
    public static final String ITEMS = "/items/";
    public static final String ORDERS = "/orders/";
    public static final String CART_ITEMS = "/cart/items";
    public static final String MAIN_ITEMS = "/main/items";
}
